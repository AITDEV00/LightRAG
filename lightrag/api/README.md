...
### Granular Reference Content Controls

By default, the `/query` and `/query/stream` endpoints return full reference details. You can selectively disable specific content types to reduce payload size using these granular parameters:

| Parameter | Default | Description |
|-----------|---------|-------------|
| `include_chunk_text` | `true` | Include chunk text content in references |
| `include_image_descriptions` | `true` | Include image descriptions in references |
| `include_entity_descriptions` | `true` | Include entity descriptions in references |

**Example API Request (exclude entity descriptions to reduce payload):**
...
**Example Response (with all content - default):**

```json
{
  "response": "LightRAG is a graph-based RAG system [1_c0]...",
  "references": [
    {
      "reference_id": "1",
      "file_path": "/documents/intro.md",
      "contents": [
        {"index": 0, "chunk_id": "chunk-abc123", "text": "LightRAG is a retrieval-augmented generation system..."}
      ],
      "images": [
        {"index": 0, "file_path": "images/diagram.jpg", "description": "System architecture diagram"}
      ],
      "entities": [
        {"index": 0, "entity_name": "LightRAG", "entity_type": "system", "description": "A graph-based RAG system..."}
      ]
    }
  ]
}
...
**Example Response (with all content stripped):**
  ...
  "references": [
    {
      "reference_id": "1",
      "file_path": "/documents/intro.md",
      "contents": [{"index": 0, "chunk_id": "chunk-abc123"}],
      "images": [{"index": 0, "file_path": "images/diagram.jpg"}],
      "entities": [{"index": 0, "entity_name": "LightRAG", "entity_type": "system"}]
    }
  ]
}
```
...
**Notes**:
- These parameters only work when `include_references=true`. Disabling content without including references has no effect.
- Use minimal data mode when you only need to track which sources were cited without displaying full content.
- You can mix and match: keep chunk text for display but exclude entity descriptions to reduce payload.
